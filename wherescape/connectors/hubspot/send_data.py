...
from hubspot.crm.companies import SimplePublicObjectInput, BatchInputSimplePublicObjectBatchInput, ApiException
...
def patch_company_on_id(id, properties, client):
    ...
    try:
        api_response = client.crm.companies.basic_api.update(company_id=id, simple_public_object_input=object_input)
...
def patch_company_batch(inputs, client):
    ...
    try:
        api_response = client.crm.companies.batch_api.update(batch_input_simple_public_object_batch_input=batch_input)
        logging.info("sending companypatch to hubspot")
        logging.debug(api_response)
        print(api_response)
    except ApiException as e:
